...
- `python -m unittest discover -s tests` �?**PASS**�?09 tests OK�?- `tests/phase2_fixture_e2e.py` �?**PASS**（PHASE2 E2E OK�?- `tests/phase1_fixture_e2e.py` �?**PASS**（PHASE1_FIXTURE_E2E_OK�?- `iterative-bug-hunter/**` 对已�?docs 路径扫描 �?**PASS**�? 命中�?- 独立 review（general-1）→ **PASS**：Spec compliance �?AC met；Correctness �?critical；Consistency 对齐
...
- 项目�?skill 副本 `.mimocode/skills/iterative-bug-hunter/`�?7 文件）与源码 `iterative-bug-hunter/` 完全重复
- `docs/compose/spec/` �?Phase0�? / real-acceptance / skill-config-opt / hunt-fix-router 等已交付历史 feature 文档
- `docs/blueprint/` 两份大体量研究蓝图（v1/v2）仍�?`references/compose-escalate.md` 硬引�?- `README.md`「仓库定位」仍把上述历�?docs 写成一等公�?- `iterative-bug-hunter/SKILL.md` frontmatter 有一处未提交的合规整理（description 单行 + compatibility 挪正文）
...
├── LICENSE
├── .gitignore                # 补齐 .mimocode/ �?├── iterative-bug-hunter/     # skill 源码（唯一真源；SKILL.md 纳入 frontmatter 整理�?├── examples/                 # 验收 demo（保留；清掉本地 .bug-hunter 运行数据�?├── tests/                    # 单元测试（保留）
└── docs/
...
| `.mimocode/skills/iterative-bug-hunter/` | 删除 | 与源码重复；安装路径写在 README 即可 |
...
| `iterative-bug-hunter/SKILL.md` | 保留未提�?frontmatter 整理 | 用户确认纳入 |

### 引用更新契约

1. `iterative-bug-hunter/references/compose-escalate.md`  
...
- [x] T3: 深度删除运行时垃圾与 skill 重复副本 �?acceptance: 主仓磁盘�?.playwright-mcp、无 .mimocode/skills/iterative-bug-hunter、examples �?.bug-hunter 运行数据�?gitignore �?.mimocode/ (covers: S2)
